...
行をまたいで続くピボットは、行ごとに分けて渡します（実質2行まで。
  schema は4行許しますが、引用が1カード1〜2行に限られるので先に効きます）。
**--repetition は渡しません。**ピボットはパターンではないので分析のフレームの
バッジを出さず（子音ピボットのガードレール3）、カードも tags.repetition を
持ちません（同2）。

  例) npm run figure -- --pivot 'カ行子音' \\
        'いつ「か」「か」ならず「か」なうって「き」め「こ」んで'
  例) npm run figure -- --pivot 'タ行子音' \\
