...
- 2014年の記事 [`sheena-ringo-repeated-consonance`](src/content/blog/2014/sheena-ringo-repeated-consonance.mdx) にある著者自作の図法に由来する（2026-08-02、やおき指摘で発見）。**ただし当時の図が持っていた下段（歌詞の全行）は持たない。** 単体で流通する OG 画像に歌詞1行を焼くことになり、公開ポリシー（1カード1〜2行以内）と衝突する。「どの語のどこか」はカード本文の `<LyricQuote>` が担う
