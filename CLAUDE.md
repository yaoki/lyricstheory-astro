...
**図の種類を増やす規則: 行数＝図に載せる箇所の数。** 1箇所なら symmetry、離れた2箇所なら pair。図の形が表すのは「何箇所載るか」であって、何を見ているか（フレーム）ではない。**3つ目の形を足す前に、この規則で説明がつくかを必ず判定する**（倒置形はこの規則で吸収できないので、足すときは別途判断が要る）。
...
- **倒置形は今の図では描けない。** 弧は「同じ音を結ぶ」だけで順序の反転を表せないため、四操作のうち実質 並行 しか図にできていない。倒置を示すには 2 列＋交差矢印の別テンプレートが要る（2026-07-30 時点で未実装）
