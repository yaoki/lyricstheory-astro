...
規則7 は引用元を `lyrics/` ディレクトリに限定しているが、その実体は `comitia156/lyrics/` にあり、**comitia156 は remote を持たないローカル repo なのでクラウドからは読めない**。検証済みソースがゼロの状態で規則7 の禁止だけが効くことになるため、クラウドでは引用元を次のとおり読み替える。

- **プロンプトに貼られたフレーズだけを引用する。** 貼られていない歌詞を一文字も書かない
- 記憶・ネット検索・一般的知識から歌詞を補完しない（規則7 の禁止はそのまま効く）
- フレーズが足りなければ、推測で埋めずに**貼り足しを依頼して止まる**
- 貼られていない箇所を「たぶんこう続く」と書かない。スピッツ「ロビンソン」の幻覚引用事件（規則7、2026-04-19）と同じ経路

規則7 の趣旨は「検証済みテキスト以外を引用しない」ことであり、その担保がファイルから貼付に移るだけである。**引用の上限（1カードあたり1〜2行）と Public 運用のガードレールは変わらない。**
