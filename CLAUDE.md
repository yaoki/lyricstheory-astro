...
**母音の図**（`kind: vowel`）は `--vowel` を付ける。音節を上段に並べ、母音だけを下段に抜き出して塗る図になる。母音は表記から自動で導かれるので、書き手は打ち込まない。

```bash
npm run figure -- --vowel 'たちど「ま」っ「た」「ま」「ま」'
```

**どちらの kind を使うか**は、何が呼応しているかで決める。音そのもの（子音＋母音）が呼応していれば `symmetry`、子音は動いていて母音だけが留まっていれば `vowel`。

- 「ん」「っ」は母音を持たないので下段が空く。長音「ー」は直前の母音を引き継ぐ
- 母音の導出は**表記ベース**。助詞の「は」は表記どおり /a/ として扱い、発音の /wa/ には寄せない。合わないときだけ `vowels:` を書いて明示する（`units` と同じ要素数にすること）
- 実装は `src/lib/og/kana.ts`。`tags.phoneme` の romaji 正規化ルールとは**別軸**なので混同しない

- **前後の文脈も枠に並べる**（対称部分だけを切り出さない）。対称がフレーズのどこで起きているかを図に残すため。2026-07-30 決定
