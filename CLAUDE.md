...
- **上限8は「文字を出す枠の数」に掛かる。伏せた枠は数に入らない。** 上限の趣旨は歌詞の連続的な再現の防止であり、伏せ字はそれに当たらない。ただし1枠4文字までは single と同じで、**連なりを1枠に畳むなら pivot ではなく single を使う**
- 総枠数は24まで。これは著作権ではなく**レイアウトの都合**（24枠で1枠約32px。カードページでは明瞭に読めるが、SNS で1/3に縮むと苦しい。16枠までが快適圏）
