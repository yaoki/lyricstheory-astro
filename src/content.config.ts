...
const singleFigure = z
  .object({
    ...
    // units の上限 12 の根拠は**判読性**であって著作権ではない（2026-08-23 変更、やおき裁定）。
    // 著作権を担うのは引用ルール（1 カード 1〜2 行）と check-cards 検査 3・4 で、図は引用済みの
    // 範囲を超えられない。pair が音数で縛られていないのと同じ根拠。正本は src/lib/og/phrase.ts の MAX_UNITS
    units: z.array(z.string().min(1).max(4)).min(2).max(12),
...
// single の上限 12 は判読性による制約で（2026-08-23 以前は著作権を根拠にしていた）、
...
const elements = defineCollection({
  ...
  schema: z.object({
    ...
    // related に混ぜると子音ピボット22枚・母音の保続16枚がそのまま流れ込み、用語カードの
