...
export interface ConsonantRow {
  ...
  units: string[];
  /**
   * 分節の位置。i は units[i] と units[i+1] のあいだを指す（2026-08-17 追加）。
   *
   * 半シラブル化のように、切れ目そのものが観察の本体になる図で使う。
   * **cuts があるときは記号の色を 1 色に揃える**。分類は記号（at・ta のような
   * ローマ字）が語るので、色には「ここが分節に参与している」だけを担わせる
   * （2026-08-17、やおき裁定。2014 年の宇多田論の図が分節された音を同じ赤で
   * 塗っていたのに倣う）。
   */
  cuts?: number[];
  /**
