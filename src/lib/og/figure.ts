...
export interface SingleFigure {
   ...
   * 呼応の弧を引くか。既定は引く。
   *
   * `false` にするのは**順序の軸が立たないカード**である。弧の向きと重なりは
   * 操作（倒置／並行）の記法なので（CLAUDE.md「図の決めごと」）、四操作が当たらない
   * カードで引くと図が本文と食い違う。単独の音の反復が二組あるカードがこれにあたる。
   *
   * 色は単独で呼応を担えるので、弧を落としても観察は読める。
   * 子音ピボットのガードレール4（弧を描かない）と同趣旨である（2026-09-05 追加）。
   */
  arcs?: boolean;
}
