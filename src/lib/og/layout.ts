...
export const SYMMETRY = {
  ...
  /** 音のベースライン */
  baseline: 320,
  /** 伏せた帯や点など「面」を薄く置く色。面積があるので縮んでも残る */
  dim: '#cbd5e1',
  /**
   * 呼応していない「字」の色。
   *
   * 面と同じ #cbd5e1 を字にも当てていたが、背景 #f8fafc に対しコントラストが 1.42:1 しかなく、
   * SNS で 1/3 に縮むと文脈が消えていた（2026-08-04 に画素で実測。3:1 を超える画素が
   * 1292 個中 0）。「前後の文脈も並べる」という決めごとが、色の設定ひとつで
   * 描画側から無効化されていたことになる。字はこちらを使う（約 2.96:1）
   */
  dimText: '#8494a8',
...
export const PAIR = {
  ...
  row2: 372,
  /**
   * 段を積むときに使ってよい縦の範囲。折り返しで段が増えると、既定の row1/row2 から
   * 等間隔に広げるだけでは下がタイトルに被る（2026-08-04 に実測）。
   * 下端はカードページが OG 画像を刈る y=400 に合わせてある
   */
  topLimit: 170,
  bottomLimit: 400,
  maxSize: 92,
...
export const PIVOT = {
   ...
   *
   * **baseline を使うのは 1 行の図だけ**（2026-09-15 から）。2 行以上の図は topLimit と題のあいだの
   * 中央に置く（`templates/pivot.ts`）。2〜4 行の baseline は、行数を増やす図を足すときの目安として残す
   */
  layouts: {
  ...
  bottomLimit: 384,
  /**
   * いちばん下の行のベースラインと、題の上端（1 行目のベースライン − titleFontSize）との距離の下限
   * （2026-09-15 追加）。
   *
   * bottomLimit だけで止めていたため、題が 2 行になると題の上端が上がり、2 行目の音と題の隙間が
   * 21〜23px まで詰まった（E159 ほか。E150 は 31px。2026-09-15 に画素で実測）。値は E150 に合わせてある
   * （最終行のベースライン 375.8、題の字の枠の上端 405）。題が 1 行なら bottomLimit のほうがきつく、効かない
   */
  titleClearance: 29,
  /**
   * 1 行目の上端（手がかりがあればその字の上端）をここより上げない（2026-09-15 追加）。
   *
   * 軸ラベル（ベースライン axisY = 112）の下に空ける余白。上端を縛っていなかったため、
   * 行の短い 2 行の図では 1 行目が bottomLimit に押し上げられ、手がかりが軸ラベルに 13px まで
   * 迫った（E159。2026-09-15 に画素で実測）。収まらないときは字を縮める（ただし minSize まで）。
   * 上端は字の枠（em）で数えるので、実際の字の上端はこれより約 4px 下になる
   */
  topLimit: 140,
  /** 枠と枠の間隔 */
  gap: 12,
  /**
   * 軸の音の最小サイズ。セル幅がこれを下回ってもここまでは大きく書く。
   * 枠数が多い行でも SNS で読めるようにするため（2026-08-04、やおき指摘）
   */
  minSize: 58,
  /** 帯として描く最小の幅。これを下回る隙間は、帯と同じ太さの点で示す */
  minMaskWidth: 22,
  /**
   * 伏せた帯の高さ＝音の大きさ × これ。
   * 固定値（26px）にしていたが、枠数が増えて字が細ると帯だけが太いまま残り、
   * 軸の音が帯の列に埋もれた（2026-08-04、やおき指摘）。字と一緒に痩せさせる
   */
  maskHeightRatio: 0.34,
  maskRadius: 6,
  /**
   * 字の視覚的な中心＝ベースライン −（音の大きさ × これ）。帯の中心をここに合わせる。
   * ベースラインからの距離を固定値（42px）にしていたため、字が小さい図では
   * 帯だけが上に浮いて行が揃わなかった（2026-08-04、やおき指摘「ベースラインが揃っていない」）
   */
  opticalCenterRatio: 0.36,
  /**
   * 伏せた枠が歌詞のどこを指しているかを示す手がかりの字の大きさ（2026-08-06 追加）。
   * 行に収まらなければここから縮める。軸の音（58〜76px）の半分ほどに留めるのは、
   * 歌詞が主役に見えると図の主従が逆になるため
   */
  hintFontSize: 34,
  /** 手がかりのベースライン＝行のベースライン −（音の大きさ × これ） */
  hintGapRatio: 0.95,
  /**
   * 手がかりを持つ行があるとき、行送りに足す高さ＝`hintFontSize` × これ（2026-08-06 追加）。
   *
   * 足さないと、2 行の図で手がかりが**自分の行より上の行に密着する**（figure-critic 判定・実測で
   * 上 6px ／ 下 9px、SNS の 1/3 では 上 1px ／ 下 2px）。縮小版では 1 行目の枠と 2 行目の
   * 手がかりが 1 つの塊に読め、どの手がかりがどの行の歌詞かという対応が壊れる。
   *
   * **`rowStepRatio` 自体を上げてはいけない。** 3〜4 行の `layouts`（baseline 205 / 170）で
   * span が伸び、1 行目が軸ラベル（`axisY` = 112）より上へ押し出される。手がかりを持つ行にだけ効かせる
   */
  hintRowStepAdd: 1.2,
  titleCenterY: 470,
  titleFontSize: 40,
