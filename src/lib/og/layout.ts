...
/**
 * 子音の並びそのものを描く図。2 つ以上の子音が交替し続ける範囲を見るときに使う。
 *
 * 単位が個々の音節ではなく子音の並びになるため、仮名の枠には載らない
 * （子音を持たない位置 Φ は仮名で書けない）。記号を直接並べる。
 *
 * **枠数の上限を持たない。** single の上限 8 は仮名が連続して並ぶことによる制約で、
 * 子音記号の列は歌詞にならないため。効くのは 1 行に入る幅だけ。
 *
 * 記号の種類ごとに色を割り当てる（出現順に ACCENTS）。Φ だけは dim に固定する。
 * 交替が色の交互として見えるようにするため。
 */
export const CONSONANT = {
  symbolFontSize: 84,
  /** 群のなかの記号と記号の間隔 */
  gap: 14,
  /** 群と群の間隔。群の区切りはこの差で示す（区切り線は引かない） */
  groupGap: 56,
  /** 1 行のときのベースライン */
  baseline: 300,
