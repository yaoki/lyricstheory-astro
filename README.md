...
図は「何が繰り返されているか」を一目で示すことを狙っている。音を横一列に並べ、呼応する音を色で立て、そのあいだを弧で結ぶ。枠を描かず文字を大きく取っているのは、SNS のタイムラインで画像が 1/3 ほどに縮むためで、装飾を削らないと一瞥で読めない。
...
- `「」` で囲んだ音が色で立ち、囲みどうしが弧で結ばれる
- 前後の文脈もそのまま並ぶ（呼応がフレーズのどこで起きているかを図に残すため）
...
**図の形は 1 種類しかない。** 何を見ているかは図の形ではなく、**左上のバッジ**が示す。`C反復`／`V反復`／`CV反復` のどのフレームで見ているかの宣言で、カードの `tags.repetition` から自動で出る。

```bash
npm run figure -- --repetition v '「き」も「ち」'
```

一曲を「C反復のフレーム」「V反復のフレーム」と決め打ちして徹底的に見る、という分析の進め方をそのまま図にしたもの。だから**個々の音に C・V を振らない**。三類型は類型であって単位ではない。

図を型ごとに分けないのは、読み手の学習コストを下げるため。**同じ形の図の色が、フレームを変えると変わる**という一貫性のほうが伝わる。
...
2. `src/lib/og/` をまるごとコピーする。Astro にも本サイトにも依存しない関数だけで、タイトルと音の列を渡すと SVG 文字列が返る
